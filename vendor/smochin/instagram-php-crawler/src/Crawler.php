...
class Crawler
{
    ...
    private function getMediaAsync(array $codes): array
    {
        ...
        foreach ($results as $r) {
            ...
            $data = json_decode($r['value']->getBody()->getContents(), true);
            $media = null;
            if (isset($data['media'])) {
                $media = $data['media'];
            } else if (isset($data['graphql']) && isset($data['graphql']['shortcode_media'])) {
                $media = $data['graphql']['shortcode_media'];

                // compatible key(s)
                $media['code'] = $media['shortcode'];
                $media['display_src'] = $media['display_url'];
                $media['date'] = $media['taken_at_timestamp'];
            }
